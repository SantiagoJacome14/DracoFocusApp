@extends('layouts.app')
@section('title', 'Teacher — Editar Pregunta')
...
<div class="min-h-screen">
        ...
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    <span class="bg-amber-400/15 border border-amber-400/30 text-amber-400 px-3 py-1 rounded-xl text-xs font-extrabold uppercase tracking-widest">📝 Panel de Profesor</span>
                </div>
                <h1 class="text-3xl font-extrabold text-white tracking-tight">Editar Pregunta</h1>
                <p class="text-slate-400 mt-1 text-sm">Actualiza los datos de la pregunta seleccionada.</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto px-8 py-8" x-data="{ qType: '{{ old('question_type', $question->question_type) }}' }">
        @if ($errors->any())
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden animate-fade-in-up p-8">
            <form action="{{ route('teacher.questions.update', $question) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="lesson_id" id="lesson_id" class="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-draco-emerald/50 focus:border-draco-emerald/50 transition-all appearance-none">
                            ...
                            @foreach($lessons as $lesson)
                                <option value="{{ $lesson->id }}" {{ old('lesson_id', $question->lesson_id) == $lesson->id ? 'selected' : '' }}>{{ $lesson->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="difficulty" id="difficulty" required class="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-draco-emerald/50 focus:border-draco-emerald/50 transition-all appearance-none">
                            ...
                            <option value="intermediate" {{ old('difficulty', $question->difficulty) == 'intermediate' ? 'selected' : '' }}>Intermedio</option>
                    ...
                    <div>
                        ...
                        <select name="lesson_type" id="lesson_type" required class="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-draco-emerald/50 focus:border-draco-emerald/50 transition-all appearance-none">
                            <option value="individual" {{ old('lesson_type', $question->lesson_type) == 'individual' ? 'selected' : '' }}>Lección Individual</option>
                            <option value="group" {{ old('lesson_type', $question->lesson_type) == 'group' ? 'selected' : '' }}>Lección Grupal</option>
                        </select>
                ...
                <div>
                    ...
                    <textarea name="question_text" id="question_text" rows="4" required placeholder="Escribe aquí la pregunta..."
                              class="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-draco-emerald/50 focus:border-draco-emerald/50 transition-all">{{ old('question_text', $question->question_text) }}</textarea>
                ...
                <div x-show="qType === 'multiple_choice'" x-transition class="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="options[{{ $letter }}]" value="{{ old('options.'.$letter, is_array($question->options) ? ($question->options[$letter] ?? '') : '') }}" 
                    ...
                    <div class="mt-6 border-t border-slate-700/50 pt-4">
                        ...
                        <select name="correct_answer" id="correct_answer_mc" x-bind:disabled="qType !== 'multiple_choice'" class="w-full bg-draco-emerald/10 border border-draco-emerald/30 rounded-xl px-4 py-3 text-draco-emerald-light focus:outline-none focus:ring-2 focus:ring-draco-emerald/50 transition-all appearance-none">
                            ...
                            <option value="A" {{ old('correct_answer', $question->correct_answer) == 'A' ? 'selected' : '' }}>Opción A</option>
                            <option value="B" {{ old('correct_answer', $question->correct_answer) == 'B' ? 'selected' : '' }}>Opción B</option>
                            <option value="C" {{ old('correct_answer', $question->correct_answer) == 'C' ? 'selected' : '' }}>Opción C</option>
                            <option value="D" {{ old('correct_answer', $question->correct_answer) == 'D' ? 'selected' : '' }}>Opción D</option>
                        </select>
                        @if($errors->has('correct_answer') && old('question_type', $question->question_type) === 'multiple_choice')
                ...
                <div x-show="qType === 'text'" x-transition class="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="correct_answer" id="correct_answer_text" x-bind:disabled="qType !== 'text'" value="{{ old('question_type', $question->question_type) === 'text' ? old('correct_answer', $question->correct_answer) : '' }}"
                        ...
                        @if($errors->has('correct_answer') && old('question_type', $question->question_type) === 'text')
                            <p class="text-red-400 text-xs font-bold mt-2">{{ $errors->first('correct_answer') }}</p>
                        @endif
                    </div>
                </div>
