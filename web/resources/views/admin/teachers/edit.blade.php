...
<div class="min-h-screen">
    ...
    <div class="px-8 py-6 border-b border-slate-700/50 bg-gradient-to-r from-slate-800/80 via-slate-900/50 to-slate-800/80" style="backdrop-filter: blur(20px);">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            <div>
                ...
                <h1 class="text-3xl font-extrabold text-white tracking-tight">Editar Profesor</h1>
                <p class="text-slate-400 mt-1 text-sm">Actualiza los datos del profesor.</p>
            </div>
    ...
    <div class="max-w-3xl mx-auto px-8 py-8">
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden animate-fade-in-up p-8">
            <form action="{{ route('admin.teachers.update', $user) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-700/50">
                    <div class="col-span-full">
                        <p class="text-amber-400/80 text-sm font-medium bg-amber-400/10 px-4 py-2 rounded-lg inline-block border border-amber-400/20">
                            💡 Deja la contraseña vacía si no deseas cambiarla.
                        </p>
                    </div>

                    <div>
                        <label for="password" class="block text-sm font-bold text-slate-300 mb-2">Nueva Contraseña <span class="text-slate-500 font-normal">(Opcional)</span></label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-bold text-slate-300 mb-2">Confirmar nueva contraseña</label>
